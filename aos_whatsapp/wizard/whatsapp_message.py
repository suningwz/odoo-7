                            ...
                            try:
                                ...
                                if record.whatsapp:                                
                                    ...
                                    if send_message.get('message')['sent']:
                                        ...
                                        vals = self._prepare_mail_message(self.env.user.partner_id.id, chatID, record and record.id, active_model, texttohtml.formatHtml(message.replace('_PARTNER_', record.name)), message_data, rec.subject, [record.id] if active_model == 'res.partner' else [record.partner_id.id], rec.attachment_ids, send_message, 'send')
                                        MailMessage.sudo().create(vals)
                                        record.chat_id = chatID
                                        _logger.warning('Success to send Message to WhatsApp number %s', whatsapp)
                                    else:
                                        vals = self._prepare_mail_message(self.env.user.partner_id.id, chatID, record and record.id, active_model, texttohtml.formatHtml(message.replace('_PARTNER_', record.name)), message_data, rec.subject, [record.id] if active_model == 'res.partner' else [record.partner_id.id], rec.attachment_ids, send_message, 'error')
                                        MailMessage.sudo().create(vals)
                                ...
                                if rec.composition_mode == 'mass_post':
                                    ...
                                    message_data = {                                        
                                        'body': message.replace('_PARTNER_', record.partner_id.name).replace('_NUMBER_', record.name).replace('_AMOUNT_TOTAL_', self.format_amount(record.amount_total, record.currency_id)),
                                    }                                                                       
                                    ...
                                    if whatsapp_sent:
                                        vals = self._prepare_mail_message(self.env.user.partner_id.id, AllchatIDs, record and record.id, active_model, texttohtml.formatHtml(message.replace('_PARTNER_', record.partner_id.name).replace('_NUMBER_', record.name).replace('_AMOUNT_TOTAL_', self.format_amount(record.amount_total, record.currency_id))), message_data, rec.subject, rec.partner_ids.ids, rec.attachment_ids, send_message, 'send')
                                        MailMessage.sudo().create(vals)
                                    else:
                                    ...
                                    for partner in rec.partner_ids:
                                        if partner.whatsapp:                                
                                            ...
                                            if send_message.get('message')['sent']:
                                                chatID = send_message.get('chatID')
                                                #vals = self._prepare_mail_message(self.env.user.partner_id.id, chatID, record and record.id, active_model, texttohtml.formatHtml(message), message_data, rec.subject, [partner.id], rec.attachment_ids, send_message, 'send')
                                                #MailMessage.sudo().create(vals)
                                                chatIDs.append(chatID)
                                                whatsapp_sent = True
                                                partner.chat_id = chatID
                                                ...
                                                #vals = self._prepare_mail_message(self.env.user.partner_id.id, chatID, record and record.id, active_model, texttohtml.formatHtml(message), message_data, rec.subject, [partner.id], rec.attachment_ids, send_message, 'error')
                                                #MailMessage.sudo().create(vals)
                                                whatsapp_sent = False
                                                _logger.warning('Failed to send Message to WhatsApp number %s', whatsapp)
                                            new_cr.commit()
                                    ...
                                    #print ('===whatsapp_sent==',whatsapp_sent)
                                    if whatsapp_sent:
                                        vals = self._prepare_mail_message(self.env.user.partner_id.id, AllchatIDs, record and record.id, active_model, texttohtml.formatHtml(message), message_data, rec.subject, rec.partner_ids.ids, rec.attachment_ids, send_message, 'send')
                                        MailMessage.sudo().create(vals)
                                    else:
                                        vals = self._prepare_mail_message(self.env.user.partner_id.id, AllchatIDs, record and record.id, active_model, texttohtml.formatHtml(message), message_data, rec.subject, rec.partner_ids.ids, rec.attachment_ids, send_message, 'error')
                                        MailMessage.sudo().create(vals)                            
                                    new_cr.commit()
                                #time.sleep(3)
